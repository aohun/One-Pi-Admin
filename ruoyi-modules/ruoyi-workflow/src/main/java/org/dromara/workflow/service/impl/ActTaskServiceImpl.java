...
public class ActTaskServiceImpl implements IActTaskService {
    ...
    public Map<String, Object> startWorkFlow(StartProcessBo startProcessBo) {
        ...
        //发起流程默认增加流水号
        variables.put("serialNumber", IdUtil.fastSimpleUUID());
        ProcessInstance pi;
        try {
    ...
    public TableDataInfo<TaskVo> getPageByAllTaskWait(TaskBo taskBo, PageQuery pageQuery) {
        ...
        if (StringUtils.isNotBlank(taskBo.getName())) {
            query.processDefinitionNameLike("%" + taskBo.getName() + "%");
...
//        if (StringUtils.isNotBlank(taskBo.getAssigneeName())) {
//            userService.selectUserNameById()
//            query.taskAssignee();
//        }
        query.includeProcessVariables();
        if(StringUtils.isNotBlank(taskBo.getSerialNumber())){
            query.processVariableValueEquals("serialNumber",taskBo.getSerialNumber());
        }
//        if(StringUtils.isNotBlank(taskBo.getAssigneeName())){
//            userService.selectUserNameById()
////            query.processVariableValueEquals("serialNumber",taskBo.getSerialNumber());
//        }
        //query.taskAssignee("1");
//        query.taskCandidateOrAssigned()
//        query.taskOwner("1");
        ...
        if (CollUtil.isNotEmpty(taskList)) {
            ...
            for (Task task : taskList) {
                ...
                //task.getCaseVariables()
                taskVo.setSerialNumber((String) task.getProcessVariables().get("serialNumber"));
