...
public class ActProcessInstanceServiceImpl implements IActProcessInstanceService {
    ...
    @Override
    public TableDataInfo<ProcessInstanceVo> getPageForHi(ProcessInstanceBo processInstanceBo, PageQuery pageQuery) {
        List<ProcessInstanceVo> list = new ArrayList<>();
        ...
        HistoricProcessInstanceQuery historicProcessInstanceQuery = historyService.createHistoricProcessInstanceQuery()
            ...
            .desc();
        List<HistoricProcessInstance> historicProcessInstances = historicProcessInstanceQuery.listPage(pageQuery.getFirstNum(), pageQuery.getPageSize());
        for (HistoricProcessInstance hisIns : historicProcessInstances) {
            ProcessInstanceVo processInstanceVo = new ProcessInstanceVo();
            processInstanceVo.setBusinessKey(hisIns.getBusinessKey());
//            processInstanceVo.setIsSuspended(processInstance.isSuspended());
            ...
            if (processStatus == null) {
                processStatus = ObjectUtil.isNull(hisIns.getEndTime()) ? "waiting" : "finish";
            }
            processInstanceVo.setBusinessStatusName(BusinessStatusEnum.findByStatus(processStatus));
            processInstanceVo.setProcessDefinitionName(hisIns.getProcessDefinitionName());
            processInstanceVo.setProcessDefinitionVersion(hisIns.getProcessDefinitionVersion());
            processInstanceVo.setProcessDefinitionId(hisIns.getProcessDefinitionId());
            ...
            if (CollUtil.isNotEmpty(taskList)) {
                if (CollUtil.isNotEmpty(taskList)) {
                    processInstanceVo.setId(taskList.get(0).getId());
                } else {
                    List<HistoricTaskInstance> historicTaskInstance = historyService.createHistoricTaskInstanceQuery().processInstanceId(hisIns.getId()).orderByHistoricTaskInstanceEndTime().desc().list();
                    processInstanceVo.setId(historicTaskInstance.get(0).getId());
                }
            }
    ...
    @Override
    public boolean suspendOrActivateProcessInstanceById(String processInstanceId) {
...
//        Task task = query.singleResult();
        ...
        if (task != null) {
            ...
            if ("2".equals(taskVo.getSuspensionState() )) {
                try{
            ...
            } else if ("1".equals(taskVo.getSuspensionState())){
                try{
                    runtimeService.suspendProcessInstanceById(processInstanceId);
                }catch (Exception e){
                    log.error("挂起失败");
                    throw new ServiceException("挂起失败:" + e.getMessage());
                }
            }
        }
